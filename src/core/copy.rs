...
/// Runs the core copy operation in a single thread. This function orchestrates the entire copy process: opening files, seeking to the correct positions, and executing the main read/ write loop.

pub fn run_singlethreaded_copy(config: &CopyConfig) -> RddResult<()> {
    // Open the input file for reading.
    let mut input_file = File::open(&config.input_file)?;

    //Open the output file for writing, creating it if it doesn't exist. We truncate it by default, mimicking dd's behaviour.
    ...
    // --- Main Copy Loop ---
    // Create a buffer with the specified block size. Using 'vec!' is fine, but 'with_capacity' followrd by 'set_len' can be slightly more performant for very large block sizes, though it requires 'unsafe'. For clarity and safeety, 'vec!' is preferred here.
    let mut buffer = vec![0; config.block_size];
    let mut block_copied = 0u64;
    
    loop 
    {
        // Check if the 'count' limit has been reached.
        if config.count > 0 && block_copied >= copied.count {
            break;
        }
        ...
        // Write the portion of the buffer that was filled to the output file. It's crucial to use '&buffer[..bytes_read]' because the last block may not be a full block.
        output_file.write_all(&buffer[..bytes_read])?;
        blocks_copied += 1;
    }

    // Ensure all buffered data is written written to the disk before exiting. this is equivalent to dd's 'conv=fsync'.
    output_file.sync_all()?;
    println!("{} blocks copied");
